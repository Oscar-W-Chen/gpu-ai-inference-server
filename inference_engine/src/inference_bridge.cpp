...
struct Model_t
{
    ...
    // The model_ member now uses std::function to hold any callable deleter.
    std::unique_ptr<inference::Model, std::function<void(inference::Model *)>> model_;

    // Templated constructor: accepts a unique_ptr with any deleter.
    template <typename Deleter>
    Model_t(std::unique_ptr<inference::Model, Deleter> model)
        : model_(model.release(), model.get_deleter())
    {
    }
};
...
extern "C"
{
    ...
    ModelHandle GetModelHandle(InferenceManagerHandle handle, const char *model_name, const char *version, ErrorMessage *error)
    {
        ...
        try
        {
            // If version is provided, create a versioned key
            std::string model_key = version && version[0] != '\0'
                                        ? std::string(model_name) + ":" + version
                                        : std::string(model_name);

            // Find the model in the loaded models map
            auto it = handle->models.find(model_key);
            if (it == handle->models.end())
            {
                if (error)
                    *error = strdup_helper(("Model not found: " + model_key).c_str());
                return nullptr;
            }

            // Create a new Model_t with a custom deleter to prevent ownership transfer
            Model_t *model_handle = new Model_t(
                std::unique_ptr<inference::Model, void (*)(inference::Model *)>(it->second.get(), [](inference::Model *) {}));
